...
2. Present:
   ...
   - A table of **cold personal skills** (`personal_cold`): name, last used ("never" when null), days cold, uses, and whether it is removable. `personal_cold_removable` of them can be disabled; the rest are report-only, because they are library skills or symlinked in from another repo. Point the user to `/skill-cleanup` to act on the removable ones, and name the owning repo for the rest.
