...
2. Present:
   ...
   - A table of **cold personal skills** (`personal_cold`): name, last used ("never" when null), days cold, uses, and whether it is `removable`. Rows with `removable: false` are report-only — library skills or skills symlinked in from another repo. Point the user to `/skill-cleanup` for the removable ones, and name the owning repo for the rest.
