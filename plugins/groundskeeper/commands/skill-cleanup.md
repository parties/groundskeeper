...
3. Let `N` be the number of `personal_cold` rows with `removable: true`. Use the **AskUserQuestion** tool — "`<N>` of `<personal_cold.length>` cold personal skills (≥`<config.cold_days>`d unused) can be disabled. How do you want to clean up?" — with options, omitting the first two when `N` is `0`:
   - **Disable all `<N>`** — prune every removable cold skill.
   - **Choose which** — pick specific skills.
   - **Snooze / keep instead** — suppress without disabling.
   - **Cancel** — do nothing.
4. **Disable all** → for each `personal_cold` row **with `removable: true`**, run `node "${CLAUDE_PLUGIN_ROOT}/scripts/audit.js" disable <skill>`. Report results.
5. **Choose which** → use AskUserQuestion with `multiSelect: true`, one option per cold skill **with `removable: true`** (label = skill name; description = `last used <last_used>, <days_cold>d cold, <uses> uses`). For each selected skill run `audit.js disable <skill>`.
